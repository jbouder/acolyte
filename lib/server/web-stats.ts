...
// Fetch the page the way the Website Analysis tool does, then derive the
// report from the raw HTML and response headers. Framework, CMS and analytics
// detection is heuristic string matching, so treat the booleans as hints.
export async function analyzeWebsite(url: string): Promise<WebsiteStats> {
  ...
  try {
    ...
    throw new ToolError('Failed to analyze website', 500);
  }
  ...
  const cssLinkCount = (html.match(/<link[^>]*rel=["']stylesheet["']/gi) || [])
    .length;
  ...
  const hasHotjar =
    html.includes('hotjar.com') || html.includes('hj(') || html.match(/\bhj\(/);
  ...
  return {
    ...
    frameworks: {
      ...
      react: Boolean(hasReact),
      vue: Boolean(hasVue),
      angular: Boolean(hasAngular),

      // CSS Frameworks
      jquery: Boolean(hasJQuery),
      bootstrap: Boolean(hasBootstrap),
      tailwind: Boolean(hasTailwind),
      materialize: Boolean(hasMaterializeCSS),
      bulma: Boolean(hasBulma),
      mui: Boolean(hasMUI),
      uswds: Boolean(hasUSWDS),

      // Full-Stack Frameworks
      nextjs: Boolean(hasNextJs),
      nuxt: Boolean(hasNuxt),
      express: Boolean(hasExpressJs),
      laravel: Boolean(hasLaravel),

      // CMS Platforms
      wordpress: Boolean(hasWordPress),
      drupal: Boolean(hasDrupal),
      joomla: Boolean(hasJoomla),

      // E-commerce Platforms
      shopify: Boolean(hasShopify),

      // Website Builders
      wix: Boolean(hasWix),
      squarespace: Boolean(hasSquarespace),
    },

    // Analytics and tracking
    analytics: {
      googleAnalytics: Boolean(hasGoogleAnalytics),
      googleTagManager: Boolean(hasGoogleTagManager),
      facebookPixel: Boolean(hasFacebookPixel),
      hotjar: Boolean(hasHotjar),
      matomo: Boolean(hasMatomo),
      amplitude: Boolean(hasAmplitude),
      segment: Boolean(hasSegment),
      zendesk: Boolean(hasZendesk),
    },
